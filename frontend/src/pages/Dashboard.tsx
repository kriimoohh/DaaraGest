...
function TauxCard({ label, data, summary }: { label: string; data?: TauxPresence; summary: (presents: number, total: number, mois: string) => string }) {
  if (!data) return (
  ...
  return (
    <div className="card" style={{ padding: '16px 18px' }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 4 }}>
        {summary(sem.presents, sem.total, pct(moisd.taux))}
      </div>
...
export function Dashboard() {
  ...
  const etabName = isAr ? (etablissement?.nom_ar || etablissement?.nom_fr) : etablissement?.nom_fr;
  ...
  return (
    <>
      ...
      <div className="page-head">
        <div>
          <div className="page-eyebrow">{t('dashboard.vue_ensemble')}</div>
      ...
      <div className="grid-4 mb-4">
        <div className="card stat">
          <div className="stat-label">
            ...
            {t('dashboard.eleves_inscrits')}
          </div>
          <div className="stat-value font-display">{fmtNum(stats.nb_eleves)}</div>
          <div className="stat-foot"><span className="muted">{t('dashboard.kpi_inscrits')}</span></div>
        </div>
        <div className="card stat">
          <div className="stat-label">
            ...
            {t('dashboard.personnel_actifs')}
          </div>
          <div className="stat-value font-display">{fmtNum(stats.nb_personnel)}</div>
          <div className="stat-foot"><span className="muted">{t('dashboard.kpi_actifs')}</span></div>
        </div>
        <div className="card stat">
          <div className="stat-label">
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                <TauxCard
                  ...
                  summary={(presents, total, mois) => t('dashboard.presence_summary', { presents, total, mois })}
                />
                <TauxCard
                  label={t('dashboard.presence_personnel')}
                  data={tdb.presence_personnel ?? tdb.presence_professeurs!}
                  summary={(presents, total, mois) => t('dashboard.presence_summary', { presents, total, mois })}
                />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 16, marginBottom: 16 }}>
                ...
                <div className="card">
                  <div className="card-hd">
                    <h3>{t('dashboard.moyennes_classes')}</h3>
                    <span className="sub">{t('dashboard.nb_classes', { count: tdb.moyennes_classes.length })}</span>
                  </div>
                  <div className="card-pad">
                    {tdb.moyennes_classes.length === 0 ? (
                      <div className="empty">{t('dashboard.pas_de_donnees')}</div>
                    ) : (
                ...
                <div className="card">
                  <div className="card-hd">
                    <h3>{t('dashboard.classement_eleves')}</h3>
                  </div>
                  <div className="card-pad" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {tdb.top5_eleves.length > 0 && (
                      <div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--success-text)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>{t('dashboard.top5')}</div>
                    ...
                    {tdb.bottom5_eleves.length > 0 && (
                      <div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--danger)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 6 }}>{t('dashboard.a_surveiller')}</div>
                    ...
                    {tdb.top5_eleves.length === 0 && tdb.bottom5_eleves.length === 0 && (
                      <div className="empty">{t('dashboard.pas_de_notes')}</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              {tdb.alertes.length > 0 && (
                <div className="card" style={{ marginBottom: 16 }}>
                  <div className="card-hd">
                    <h3>{t('dashboard.alertes_actives')}</h3>
                    <span className="badge badge-danger"><span className="badge-dot" />{tdb.alertes.length}</span>
                  </div>
                  <div className="tbl-wrap">
                    <table className="tbl">
                      <thead>
                        <tr><th>{t('dashboard.table_type')}</th><th>{t('dashboard.table_eleve')}</th><th>{t('dashboard.table_matricule')}</th><th>{t('dashboard.table_valeur')}</th></tr>
                      </thead>
                      <tbody>
                        {tdb.alertes.map((a, i) => (
                          <tr key={i}>
                            <td>
                              {a.type === 'absences_repetees'
                                ? <span className="badge badge-warning"><span className="badge-dot" />{t('dashboard.absences_repetees')}</span>
                                : <span className="badge badge-danger"><span className="badge-dot" />{t('dashboard.note_insuffisante')}</span>
                              }
                            </td>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }} className="mb-4">
        <div className="card">
          <div className="card-hd">
            <div>
              <h3>{t('dashboard.encaissements_6_mois')}</h3>
              {statsMensuels.length > 0 && (
                <div className="sub mt-1">
                  {t('dashboard.total_label')} <span className="font-mono">{fmtNum(statsMensuels.reduce((s, m) => s + m.total, 0))} FCFA</span>
                </div>
              )}
            </div>
          ...
          <div className="card-pad">
            {statsMensuels.length > 0 ? (
              ...
              <div className="empty">{t('dashboard.aucune_donnee_finance')}</div>
            )}
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-hd">
            <h3>{t('dashboard.ce_mois')}</h3>
            <span className="sub">{new Date().toLocaleDateString(locale, { month: 'long', year: 'numeric' })}</span>
          </div>
          <div className="card-pad" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {statsMois ? (
              <>
                <div>
                  <div className="info-label">{t('dashboard.encaisse_eleves')}</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, fontSize: 12 }}>
                  ...
                  <div>
                    <div className="info-label">{t('dashboard.verse_personnel')}</div>
              ...
              <div className="empty" style={{ padding: '24px 0' }}>{t('common.chargement')}</div>
            )}
          </div>
        </div>
      </div>
