...
export async function demandesAbsenceProfRoutes(fastify: FastifyInstance) {
  fastify.get('/', { preHandler: [authMiddleware, acces] }, async (req, _reply) => {
  ...
  fastify.post('/', { preHandler: [authMiddleware, acces] }, async (req, reply) => {
    const user = req.user as { etablissement_id: string };
  ...
  fastify.patch('/:id/traiter', { preHandler: [authMiddleware, direction] }, async (req, _reply) => {
  ...
  fastify.delete('/:id', { preHandler: [authMiddleware, acces] }, async (req, reply) => {
    const user = req.user as { etablissement_id: string };
    const { id } = req.params as { id: string };
