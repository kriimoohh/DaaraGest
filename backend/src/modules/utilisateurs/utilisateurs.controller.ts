...
export async function creerHandler(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    const data = await creerUtilisateur(etablissement_id, parsed.data);
...
export async function modifierHandler(
  ...
  try {
    const data = await modifierUtilisateur(id, etablissement_id, parsed.data);
    return reply.send(data);
  } catch (err) {
...
export async function supprimerHandler(
  ...
  const { id } = request.params as { id: string };
  try {
    await supprimerUtilisateur(id, etablissement_id);
...
export async function resetPasswordHandler(
  ...
  try {
    const data = await resetPassword(id, etablissement_id, parsed.data);
    return reply.send(data);
  } catch (err) {
    return reply.status(404).send({ error: (err as Error).message });
  }
